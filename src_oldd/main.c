...
/*   Updated: 2023/10/06 16:53:19 by liurne           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char** av, char **envp)
{
	...
	while (ft_strcmp(prompt.line.line, "exit"))
	{
		...
		if (prompt.line.line[0])
			add_history(prompt.prompt.line);
		pars(&prompt);
		if (!ft_strcmp(prompt.prompt.line, "pwd"))
		{
			char	*pwd = getcwd(NULL, 0);
			ft_dprintf(2, "%s\n", pwd);
			free(pwd);
		}
		if (!ft_strcmp(prompt.prompt.line, "slt"))
			ft_dprintf(2, "%s %d\n", ", ca va ?", 8);
	}
	printf(RED"bye bye patate!!\n"END);
	free(prompt.prompt.line);
	return (0);
}